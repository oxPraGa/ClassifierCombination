...
function afficher_resultat(xml,id){
    var xmlDoc;
      var T=[];
    ...
    VMx = xmlDoc.getElementsByTagName(id)[0];
    VM=document.getElementById(id);
    classe = VMx.getElementsByTagName('classe')[0].firstChild.nodeValue;
    bel = VMx.getElementsByTagName('bel')[0].firstChild.nodeValue;
    VM.getElementsByClassName('classe')[0].innerHTML=T[classe];
    VM.getElementsByClassName('bel')[0].innerHTML=bel;
}
    function  traite_xml(xml){

       afficher_resultat(xml,'nn_lm'); 
       afficher_resultat(xml,'nn_ft'); 
       afficher_resultat(xml,'svm_lm'); 
       afficher_resultat(xml,'svm_ft'); 
        
       afficher_resultat(xml,'voteMajoritaire'); 
       afficher_resultat(xml,'borda'); 
       afficher_resultat(xml,'BKS'); 
       afficher_resultat(xml,'DS'); 
      
    }


    function LireEcrire_result(){
  ...
  xmlhttp.open("GET", config["matlab"] + "result.xml ?" + new Date().getTime(), true);
  xmlhttp.send();
    }
